...
function checkScreenSize() {
  graphWidth = canvas.node().clientWidth - margin.left - margin.right;
 
  // Update all graphWidth related stuff
  // Update svg width
  svg.attr("width", graphWidth + margin.left + margin.right);
  // Update x scale range
  x.range([0, graphWidth]);
  // Update circles
  graph.selectAll("circle").attr("cx", d => x(new Date(d.date)));
  // Update path
  path.attr("d", line);
  // Update x axis
  const xAxis = d3
...
const update = data => {
  ...
  circles.attr("cy", d => y(d.distance));
